...
/// [`jax_i18n_string_free`].
#[unsafe(no_mangle)]
pub extern "C" fn jax_i18n_core_version() -> *const c_char {
    VERSION_C_STRING.as_ptr().cast()
}

/// Return the current JSON manifest schema version.
#[unsafe(no_mangle)]
pub extern "C" fn jax_i18n_manifest_schema_version() -> u32 {
...
///   once with [`jax_i18n_string_free`].
#[unsafe(no_mangle)]
pub unsafe extern "C" fn jax_i18n_scan_archive_json(
...
/// behavior.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn jax_i18n_string_free(value: *mut c_char) {
    if value.is_null() {
        return;
    }
...
#[unsafe(no_mangle)]
pub extern "C" fn jax_i18n_last_error_code() -> i32 {
...
/// with [`jax_i18n_string_free`]. A null result means allocation/conversion
/// panicked; no panic crosses the ABI.
#[unsafe(no_mangle)]
pub extern "C" fn jax_i18n_last_error_message() -> *mut c_char {
    catch_unwind(|| {
...
mod tests {
    use super::{
        jax_i18n_core_version, jax_i18n_last_error_code, jax_i18n_last_error_message,
        jax_i18n_scan_archive_json, jax_i18n_string_free,
    };
    ...
    fn version_is_static_utf8() {
        let pointer = jax_i18n_core_version();
    ...
    fn null_input_sets_thread_local_error() {
        ...
        let code = unsafe { jax_i18n_scan_archive_json(ptr::null(), &mut output) };
        assert_eq!(code, ErrorCode::NullPointer as i32);
        assert_eq!(jax_i18n_last_error_code(), ErrorCode::NullPointer as i32);
        assert!(output.is_null());

        let message = jax_i18n_last_error_message();
        ...
        unsafe { jax_i18n_string_free(message) };
    }
}
