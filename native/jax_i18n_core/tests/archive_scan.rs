...
use jax_i18n_core::{
    ...
    ffi::{jax_i18n_last_error_code, jax_i18n_scan_archive_json, jax_i18n_string_free},
...
impl TestArchive {
    fn new(file_name: &str) -> Self {
        ...
        let directory = std::env::temp_dir().join(format!(
            "jax-i18n-core-test-{}-{counter}-{nanos}",
            std::process::id()
        ));
...
fn c_abi_returns_owned_json_and_reports_errors() {
    ...
    let code = unsafe { jax_i18n_scan_archive_json(path.as_ptr(), &mut output) };
    assert_eq!(code, ErrorCode::Ok as i32);
    assert_eq!(jax_i18n_last_error_code(), ErrorCode::Ok as i32);
    ...
    unsafe { jax_i18n_string_free(output) };
    ...
    let code = unsafe { jax_i18n_scan_archive_json(missing.as_ptr(), &mut failed_output) };
    assert_eq!(code, ErrorCode::Io as i32);
    assert!(failed_output.is_null());
}

fn assert_limit_error(result: Result<jax_i18n_core::ScanManifest, CoreError>) {
