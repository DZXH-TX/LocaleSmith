...
{
    public const string DefaultAssociatedDataNamespace = "LocaleSmith.Config";
    private const int NonceLength = 12;
    private const int TagLength = 16;
    private const long MaximumFileLength = 16 * 1024 * 1024;
    private readonly string _filePath;
    private readonly string _purpose;
    private readonly string _associatedDataNamespace;
    ...
    public EncryptedJsonConfigurationStore(
        ...
        string associatedDataNamespace = DefaultAssociatedDataNamespace)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(filePath);
        ArgumentException.ThrowIfNullOrWhiteSpace(purpose);
        ArgumentException.ThrowIfNullOrWhiteSpace(associatedDataNamespace);
        _filePath = Path.GetFullPath(filePath);
        _purpose = purpose;
        _associatedDataNamespace = associatedDataNamespace;
    ...
    {
        await SaveCoreAsync(configuration, overwrite: true, cancellationToken).ConfigureAwait(false);
    }

    public Task<bool> SaveIfAbsentAsync(
        TConfiguration configuration,
        CancellationToken cancellationToken = default) =>
        SaveCoreAsync(configuration, overwrite: false, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                try
                {
                    return await WriteAtomicallyAsync(envelopeBytes, overwrite, cancellationToken).ConfigureAwait(false);
                }
                finally
                {
                    CryptographicOperations.ZeroMemory(envelopeBytes);
    ...
    private byte[] GetAssociatedData() =>
        Encoding.UTF8.GetBytes($"{_associatedDataNamespace}|v1|{_purpose}");
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            try
            {
                File.Move(temporaryPath, _filePath, overwrite);
                return true;
            }
